# AGENTS.md — Project Handoff & Context

> Authoritative context for Codex sessions on Shyon Shiri's portfolio.
...
Older standalone pages, unused assets, original PNGs and the development scratchpad are archived at
`/Users/shyonshiri/Desktop/Shyon Website Archive - 2026-09-14`. See its manifest and README to restore files.
Historic `scratchpad/` references below point into that archive; scripts need their original project-relative paths when restored.

Design direction: dark, minimal, editorial. Custom cursor, restrained motion, system sans-serif typography with Inter fallback. The Realm is a moody, tactile LEGO world built entirely from Shyon's own Blender assets.
...
- `src/App.tsx` — the main site shell and most pages. No router; `useState<Page>` switches pages.
  `src/components/AboutPage.tsx` and `SelectedWork.tsx` own the redesigned About page and Home project previews, each with scoped CSS.
...
- `public/assets/` — current site media and active `.glb` models. `public/assets/story/` — storyboard stills. Seven PNGs are now pixel-identical lossless WebP files, with the originals archived. Everly previews use 640/1280px variants; the project viewer retains the full image.
- `scripts/check-assets.mjs` — validates literal local assets and explicit dynamic filename patterns before deployment.
- Off-screen storyboard images lazy-load. Work canvas pauses in hidden tabs. Legacy unused CSS and inert wheel/touch navigation handlers were removed without changing navigation behavior.
...
Use the Browser pane's `preview_start` with a config from `.Codex/launch.json`, never Bash.
...
`npm run deploy` now validates TypeScript and asset references, builds, and stages source (`src/`, `public/`, `index.html`) together with `docs/` before committing and pushing. Stage specific configuration/documentation/script changes separately before deployment. `npm run check` is the reusable preflight. See README.md for maintenance instructions.
...
It began as the main site's own circle dot (`#ss-cursor-dot` in `src/App.tsx`). It was ported
BACK over that site on 2026-09-09, on measurements of its `difference` dot inverting to magenta
over the Work cards and fading to 26.8 dLum on mid tones, and **THAT PORT WAS REVERTED ON
2026-09-10 at Shyon's request**: the main site is the plain white difference dot again and this
shape is the Realm's alone, so the two surfaces do NOT share a pointer. Nothing here changed,
and the three behavioural fixes that came out from under the port stayed on the main site (see
the pointer entry in §6). The dot itself
...
**THE MAIN SITE'S POINTER IS THE PLAIN WHITE DIFFERENCE DOT. IT WORE THE REALM'S GLASS
BUBBLE FOR ONE DAY, AND FOUR SEPARATE DEFECTS WERE UNDER IT** (user, 2026-09-09: the cursor
does not look right entirely, and sometimes glitches to the original mouse; reverted
2026-09-10 on "change the style of the cursor back to what we had prior"). Every one was
found by measurement. Do not tune any of this by eye.
**THE LOOK WENT BACK AND THE FIXES DID NOT, and that split is the entry.** Only ONE of the
four was the dot's appearance. The other three are behaviour and are still in the file: the
pointer at every desktop width, the cancelled native drag, and the reveal on real movement.
A revert that had taken the whole commit would have handed back a site with no pointer under
768px and the arrow flashing back mid drag, which is what Shyon complained about in the first
place. `scratchpad/cursor_revert.cjs` is the check for exactly that split, and it is a
SEPARATE script rather than an edit to `cursor_check.cjs`, which is the record of the bubble
pass and two of whose assertions (the rim, the hover swell) no longer have a subject.
...
· **THE DIFFERENCE BLEND IS BACK, AND WHAT IS WRONG WITH IT IS STILL TRUE.** A difference
  dot over backdrop B composites to `B + a*(255-2B)`, so it INVERTS: it renders MAGENTA over
  the green card artwork and cyan over the studio wall, i.e. it is a different colour on every
  ground rather than one object. And it cancels exactly at B=127.5, so it thins toward mid
  tones: **26.8 dLum** measured over the Work card art in the pass that replaced it, against
  200+ on the flat page grounds, and Work is the one page made entirely of mid-tone imagery.
  That is the known cost of this look and it is ACCEPTED, not overlooked. **Do not "fix" it by
  putting a rim or a drop shadow on it**: a difference dot with either is neither thing, and
  the fix that was already built for this is the bubble, which is still in the Realm and in
  git (`2960323`). The bubble's own numbers, if it is ever wanted back: separation 53.5 on the
  studio wall, 57.8 on the cream column, 137 on the sky glow, 179.9 on the card art.
  **The revert's check scores each ground against its OWN ceiling, `|255-2B|`, not against a
  flat floor**, because a floor would be asserting against the look that was asked for.
  Measured after: 162.6 of 164.8 on the studio wall, 206 of 207.6 on the cream column, 250 of
  247 on the black jacket, 91.4 of 126.4 on the card art. `scratchpad/cursor_shot.cjs` renders
  a sheet at 11x over every ground, because nine pixels cannot be judged at actual size.
· **THE HOVER STATE IS WIRED EVERYWHERE AND STYLED NOWHERE AGAIN, DELIBERATELY.**
  `useCursorHover` puts `.ss-hover` on the body from links, cards, nav items and buttons across
  all four pages, and no rule names it, so the pointer does not react to what it is over. That
  is what the prior pointer did, so the revert took the swell with the shape. The hook is
  untouched, so a hover state is one rule away if it is ever wanted. **A scale is the one thing
  not to reach for on this dot**: 2.2x of pure inversion is a blob rather than a pointer.
...
`Slide` reveals once with short grouped entrances. `Words` keeps the individual spans for wrapping,
but animates a whole heading or paragraph together. The hero's per-letter animation is separate.
Older six-slide and per-word timing assertions need updating before reuse.
...
**The hero describes the interactive 3D environment on every device.** Only entry is gated;
the named hero link scrolls to the introduction, so a phone can still read about the Realm.
...
cue gets real padding instead, being a lone glyph. The hero now uses two named buttons with
...
professional identity; present it as portfolio work, not as an active business the user currently runs. The user does not want
the AI evaluation job emphasized or included on About; career details belong in the résumé. No project
selector, discipline list, repeated process section, or Work CTA. Keep each fact in one place.
Home carries selected project teasers (title and role only) and the Realm project walkthrough;
Work carries the catalog and detailed descriptions; Contact carries availability and links.
The Realm origin slide was removed. Section headings are “My Lego Realm”, “Navigation”, and “Development”.
The final panel has no extra headline; its action is “Enter the Realm”. Do not reintroduce colloquial
headings such as “Under the hood” or generic designer slogans.
