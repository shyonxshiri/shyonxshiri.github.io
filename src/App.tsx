...
function MediaViewer({ item, onClose, onItemClick }: { item: MediaItem; onClose: () => void; onItemClick?: (item: MediaItem) => void }) {
  ...
  // Build sequences always open at the original. Other media keep their own controls.
  const [buildSelection, setBuildSelection] = useState({ src: item.src, index: 0 });
  useEffect(() => { setBuildSelection({ src: item.src, index: 0 }); }, [item.src]);
  const buildIndex = buildSelection.src === item.src ? buildSelection.index : 0;
  const build = item.builds?.[buildIndex];
  const displayedItem = build ? { ...item, ...build } : item;
    ...
    >
      ...
      <div onClick={e => e.stopPropagation()} className="ss-media-viewer">
        ...
        <div className="ss-viewer-media">
          ...
          {item.builds && (
            <div className="ss-booklet-controls" role="group" aria-label={`${item.title} build comparison`}>
              <button aria-label="Show original build" disabled={buildIndex === 0}
                onClick={() => setBuildSelection({ src: item.src, index: buildIndex - 1 })} {...hover}>←</button>
              <span aria-live="polite">{build?.label} · {buildIndex + 1} of {item.builds.length}</span>
              <button aria-label="Show redesigned build" disabled={buildIndex === item.builds.length - 1}
                onClick={() => setBuildSelection({ src: item.src, index: buildIndex + 1 })} {...hover}>→</button>
            </div>
          )}
        </div>
